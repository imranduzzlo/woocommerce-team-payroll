...
class WC_Team_Payroll_MyAccount {
		...
		<div class="pv-page-wrapper wc-team-payroll-reports">
			...
			<div class="performance-tracker-wrapper pv-section-wrapper" id="performance-tracker-container">
				...
				<div class="performance-tabs">
					...
					<button class="performance-tab" data-tab="period_achievements">
	...
	private static function get_employee_header( $user_id ) {
		...
		if ( ! empty( $period_stats ) && isset( $period_stats['highest_tier'] ) ) {
			...
			// Get star count for the highest tier
			if ( isset( $period_stats['tier_categories'][ $highest_tier ] ) ) {
				$tier_star_count = count( $period_stats['tier_categories'][ $highest_tier ] );
			}
		}
...
class WC_Team_Payroll_MyAccount {
	...
	public static function ajax_export_filtered_report() {
		...
		try {
			...
			if ( $format === 'csv' ) {
				$content = self::generate_csv_content( $filtered_orders, $user_name, $start_date, $end_date );
				$filename = 'reports_' . sanitize_file_name( $user_name ) . '_' . date( 'Y-m-d' ) . '.csv';
				...
				$html_content = self::generate_pdf_html( $filtered_orders, $user_name, $start_date, $end_date, $earnings_data );
				$filename = 'reports_' . sanitize_file_name( $user_name ) . '_' . date( 'Y-m-d' ) . '.html';
				
				// Send HTML for PDF conversion
		...
		wp_send_json_error( __( 'Export failed', 'wc-team-payroll' ) );
	}

	/**
	 * Generate CSV content
	 */
	private static function generate_csv_content( $orders, $user_name, $start_date, $end_date ) {
	 ...
	 */
	private static function generate_pdf_html( $orders, $user_name, $start_date, $end_date, $earnings_data ) {
		// Calculate totals
		$total_earnings = 0;
		$total_commission = 0;
		$total_orders = count( $orders );
		...
		<div class="order-details-wrapper">
			...
			<div id="order-info-tab" class="order-tab-content active">
				<div class="order-details-content">
					...
					<div class="order-header-section">
						<div class="order-basic-info">
							<div class="info-grid">
								...
								<div class="info-item">
									<span class="label"><i class="ph ph-calendar"></i> <?php esc_html_e( 'Date', 'wc-team-payroll' ); ?></span>
									<span class="value"><?php echo esc_html( $order->get_date_created()->format( 'F j, Y g:i A' ) ); ?></span>
								</div>
								<div class="info-item">
								...
								<div class="info-item">
									<span class="label"><i class="ph ph-currency-circle-dollar"></i> <?php esc_html_e( 'Total', 'wc-team-payroll' ); ?></span>
									<span class="value amount"><?php echo wp_kses_post( wc_price( $order->get_total() ) ); ?></span>
								</div>
								<div class="info-item">
									<span class="label"><i class="ph ph-credit-card"></i> <?php esc_html_e( 'Payment Method', 'wc-team-payroll' ); ?></span>
									<span class="value"><?php echo esc_html( $order->get_payment_method_title() ?: __( 'N/A', 'wc-team-payroll' ) ); ?></span>
								</div>
								<div class="info-item">
									<span class="label"><i class="ph ph-truck"></i> <?php esc_html_e( 'Shipping Method', 'wc-team-payroll' ); ?></span>
									<span class="value"><?php echo esc_html( $order->get_shipping_method() ?: __( 'N/A', 'wc-team-payroll' ) ); ?></span>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="section-content">
							<div class="info-grid">
								...
								<?php if ( $order->get_billing_company() ) : ?>
									<div class="info-item">
										<span class="label"><?php esc_html_e( 'Company', 'wc-team-payroll' ); ?></span>
										<span class="value"><?php echo esc_html( $order->get_billing_company() ); ?></span>
									</div>
								<?php endif; ?>
							</div>
						</div>
					...
					<!-- Billing Address -->
					<div class="order-section">
						<h4><i class="ph ph-map-pin"></i> <?php esc_html_e( 'Billing Address', 'wc-team-payroll' ); ?></h4>
						<div class="section-content">
							<div class="address-block">
								<?php echo wp_kses_post( $order->get_formatted_billing_address() ?: __( 'N/A', 'wc-team-payroll' ) ); ?>
							</div>
						</div>
					</div>
					...
					<?php if ( $order->get_formatted_shipping_address() ) : ?>
						<div class="order-section">
							<h4><i class="ph ph-package"></i> <?php esc_html_e( 'Shipping Address', 'wc-team-payroll' ); ?></h4>
							<div class="section-content">
								<div class="address-block">
								...
								<tbody>
									<?php foreach ( $order->get_items() as $item_id => $item ) : ?>
										<tr>
											...
											<td><?php echo wp_kses_post( wc_price( $order->get_item_subtotal( $item, false, false ) ) ); ?></td>
											<td><?php echo wp_kses_post( wc_price( $order->get_line_subtotal( $item, false, false ) ) ); ?></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
								<tfoot>
									...
									<tr class="order-total-row">
										<th colspan="3"><?php esc_html_e( 'Total', 'wc-team-payroll' ); ?></th>
					...
					?>
						<div class="order-section">
							<h4><i class="ph ph-note"></i> <?php esc_html_e( 'Customer Notes', 'wc-team-payroll' ); ?></h4>
							<div class="section-content">
								<div class="notes-list">
									<?php foreach ( $notes as $note ) : ?>
										<div class="note-item">
											<div class="note-meta">
												<span class="note-date"><?php echo esc_html( date_i18n( 'F j, Y g:i A', strtotime( $note->date_created ) ) ); ?></span>
											</div>
				...
				<div class="order-changelog-content">
					<div class="changelog-header">
						<p class="changelog-description">
							<i class="ph ph-info"></i>
							<?php esc_html_e( 'Track all changes made to this order including status updates, note additions, and other modifications.', 'wc-team-payroll' ); ?>
						</p>
					</div>
					
					<div class="changelog-timeline">
						<?php
						// Get all order notes (including system notes)
						...
						if ( ! empty( $all_notes ) ) :
							foreach ( $all_notes as $note ) :
								$note_type = $note->customer_note ? 'customer' : 'system';
								$note_icon = $note->customer_note ? 'ph-chat-circle-text' : 'ph-gear';
								?>
								<div class="changelog-entry <?php echo esc_attr( $note_type ); ?>-note">
									<div class="changelog-icon">
										<i class="ph <?php echo esc_attr( $note_icon ); ?>"></i>
									</div>
									<div class="changelog-details">
										<div class="changelog-meta">
											<span class="changelog-date"><?php echo esc_html( date_i18n( 'F j, Y g:i A', strtotime( $note->date_created ) ) ); ?></span>
											<span class="changelog-type-badge <?php echo esc_attr( $note_type ); ?>">
												<?php echo $note->customer_note ? esc_html__( 'Customer Note', 'wc-team-payroll' ) : esc_html__( 'System Note', 'wc-team-payroll' ); ?>
											</span>
										</div>
										...
										<?php if ( $note->added_by ) : ?>
											<div class="changelog-author">
												...
												if ( $author ) {
													echo '<i class="ph ph-user"></i> ' . esc_html__( 'By:', 'wc-team-payroll' ) . ' ' . esc_html( $author->display_name );
												}
												?>
											</div>
										<?php endif; ?>
