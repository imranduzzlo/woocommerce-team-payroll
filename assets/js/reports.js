...
jQuery(document).ready(function($) {
	...
	$('#reports-print-report').on('click', function() {
		// Print the current filtered report
		const printWindow = window.open('', '', 'height=600,width=800');
		const reportContent = document.querySelector('.reports-table-wrapper');
		...
		if (reportContent) {
			...
			alert('No report data to print. Please load the report first.');
		}
	});
			...
			success: function(response) {
				...
				if (response.success) {
					...
					if (format === 'csv') {
						...
						// For PDF, open in new window for user to save
