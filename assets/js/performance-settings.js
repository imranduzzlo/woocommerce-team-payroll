...
jQuery(document).ready(function($) {
	...
	// Export Settings
	$('#wc-tp-export-settings').on('click', function() {
		const $button = $(this);
		const $status = $('#wc-tp-export-import-status');
		
		$button.prop('disabled', true);
		$button.html('<span class="dashicons dashicons-update-alt" style="animation: spin 1s linear infinite;"></span> ' + wcTPPerformanceSettings.exporting);
		
		$.ajax({
			url: ajaxurl,
			type: 'POST',
			data: {
				action: 'wc_tp_export_settings',
				nonce: wcTPPerformanceSettings.nonce
			},
			success: function(response) {
				if (response.success) {
					...
					$status.html('<div class="notice notice-success"><p>' + wcTPPerformanceSettings.exportSuccess + '</p></div>').slideDown();
					setTimeout(() => $status.slideUp(), 3000);
				} else {
					$status.html('<div class="notice notice-error"><p>' + response.data.message + '</p></div>').slideDown();
				}
			},
			error: function() {
				$status.html('<div class="notice notice-error"><p>' + wcTPPerformanceSettings.exportError + '</p></div>').slideDown();
			},
			complete: function() {
				$button.prop('disabled', false);
				$button.html('<span class="dashicons dashicons-download"></span> ' + wcTPPerformanceSettings.exportButton);
			}
		});
	});

	// Import Settings
	$('#wc-tp-import-settings').on('click', function() {
		$('#wc-tp-import-file').click();
	});

	$('#wc-tp-import-file').on('change', function() {
		const file = this.files[0];
		if (!file) return;

		const $status = $('#wc-tp-export-import-status');
		const formData = new FormData();
		formData.append('action', 'wc_tp_import_settings');
		formData.append('nonce', wcTPPerformanceSettings.nonce);
		formData.append('settings_file', file);

		$status.html('<div class="notice notice-info"><p>' + wcTPPerformanceSettings.importing + '</p></div>').slideDown();
		...
		$.ajax({
			...
			success: function(response) {
				if (response.success) {
					$status.html('<div class="notice notice-success"><p>' + response.data.message + '</p></div>').slideDown();
					setTimeout(() => {
						location.reload();
					}, 2000);
				} else {
			...
			error: function() {
				$status.html('<div class="notice notice-error"><p>' + wcTPPerformanceSettings.importError + '</p></div>').slideDown();
			},
			complete: function() {
				// Reset file input
				$('#wc-tp-import-file').val('');
			}
		});
	});
